...
base_cost_file = (
    BASE_DIR / "260110_Cost of electricity generation_PHL_Base_v8_updated.xlsx"
)
pep_cost_file = (
    PEP_DIR / "260111_Cost of electricity generation_PHL_PEP_v8.xlsx"
)
...
def read_cost_data(file_path, sheet_name, row_index=5):
    ...
    return pd.Series(
        values, index=years, name="Average electricity cost (USD/kWh)"
    )
...
def read_investments_data(file_path, sheet_name):
    ...
    return pd.Series(
        values, index=years, name="Total Annualized Investments (Million USD)"
    )
...
def create_plots():
    ...
    # Investments
    base_investments = read_investments_data(
        base_cost_file, "Investments_Base_v8"
    )
    ...
    investments_pct_change = calculate_percentage_change(
        base_investments, pep_investments
    )
    ...
    fig.suptitle(
        "Percentage Change: PEP vs Baseline Scenarios",
        fontsize=16,
        fontweight="bold",
    )
    ...
    ax1 = axes[0, 0]
    ax1.plot(
        cost_pct_change.index,
        cost_pct_change.values,
        marker="o",
        linewidth=2,
        markersize=4,
        color="#2E86AB",
    ...
    ax1.set_title(
        "Average Electricity Cost (USD/kWh)", fontsize=13, fontweight="bold"
    )
    ...
    ax3 = axes[1, 0]
    ax3.plot(
        co2e_pct_change.index,
        co2e_pct_change.values,
        marker="^",
        linewidth=2,
        markersize=4,
        color="#F18F01",
    ...
    ax4 = axes[1, 1]
    ax4.plot(
        pm25_pct_change.index,
        pm25_pct_change.values,
        marker="d",
        linewidth=2,
        markersize=4,
        color="#6A994E",
    ...
    print(
        f"   Min % change: {cost_pct_change.min():.2f}% (Year {cost_pct_change.idxmin()})"
    )
    print(
        f"   Max % change: {cost_pct_change.max():.2f}% (Year {cost_pct_change.idxmax()})"
    )

    print("\n2. Total Annualized Investments:")
    print(f"   Mean % change: {investments_pct_change.mean():.2f}%")
    print(
        f"   Min % change: {investments_pct_change.min():.2f}% (Year {investments_pct_change.idxmin()})"
    )
    print(
        f"   Max % change: {investments_pct_change.max():.2f}% (Year {investments_pct_change.idxmax()})"
    )

    print("\n3. CO2e Emissions:")
    print(f"   Mean % change: {co2e_pct_change.mean():.2f}%")
    print(
        f"   Min % change: {co2e_pct_change.min():.2f}% (Year {co2e_pct_change.idxmin()})"
    )
    print(
        f"   Max % change: {co2e_pct_change.max():.2f}% (Year {co2e_pct_change.idxmax()})"
    )

    print("\n4. PM2.5 Emissions:")
    print(f"   Mean % change: {pm25_pct_change.mean():.2f}%")
    print(
        f"   Min % change: {pm25_pct_change.min():.2f}% (Year {pm25_pct_change.idxmin()})"
    )
    print(
        f"   Max % change: {pm25_pct_change.max():.2f}% (Year {pm25_pct_change.idxmax()})"
    )
    print("=" * 60)
